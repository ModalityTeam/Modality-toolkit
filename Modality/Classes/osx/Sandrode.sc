...
Sandrode { 
	...
	classvar <>sameWeight = 10, <>otherWeight = 1;
	...
	*mtpKeydownFunc { 
		^{ |view, key| 	// for some reason, the view handed in is the userView - WTF???
			...
			if (key == $.) { MultiTouchPad.stop };
			if (key == $ ) { MultiTouchPad.start };
				// fullscreen goes black after the first time on my PB
		//	if (key == $m) { w.fullScreen; };
		//	if (key == $x) { w.endFullScreen.bounds_(Rect(100, 100, 525, 375)); }; 
			if (key == $m) { 
			...
			if (key == $d) { Sandrode.drawAll(w.view) };
		}
	}
}
